/*  =========================================================================
    fty_metric_composite_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    ...
    { "comm_subprocess", NULL, true, false, "comm_subprocess_test" },
...
{
    test_item_t *item;
    printf ("Running fty-metric-composite selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_METRIC_COMPOSITE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_composite_private_selftest (verbose, item->subtest);
#endif // FTY_METRIC_COMPOSITE_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_metric_composite_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-metric-composite test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_METRIC_COMPOSITE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_composite_private_selftest (verbose, test->subtest);
#endif // FTY_METRIC_COMPOSITE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
